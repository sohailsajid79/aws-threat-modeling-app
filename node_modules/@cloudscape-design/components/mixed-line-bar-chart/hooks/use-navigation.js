...
export function useNavigation({ series, visibleSeries, scaledSeries, barGroups, xScale, yScale, highlightedPoint, highlightedGroupIndex, highlightedSeries, isHandlersDisabled, pinPopover, highlightSeries, highlightGroup, highlightPoint, highlightX, verticalMarkerX, isRtl, horizontalBars, }) {
    ...
    const moveBetweenSeries = useCallback((direction, startFrom) => {
        ...
        if (nextSeries.type === 'line') {
            ...
            const scaledTargetIndex = scaledSeries.map(it => { var _a; return ((_a = it.datum) === null || _a === void 0 ? void 0 : _a.x) || null; }).indexOf(actualTargetX);
            highlightPoint({
                x: targetXPoint,
